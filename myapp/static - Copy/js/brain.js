...
// const note_stu_overlay = document.querySelector(".note_stu_overlay");
    ...
    try {
        ...
        copy_btn.addEventListener("click", function () {
            navigator.clipboard.writeText(user_content.innerText).then(
                function () {
                    Swal.fire({ icon: "success", title: "Copied", text: "Message copied to clipboard", theme: "dark" });
                },
                function (err) {
                    console.error("Could not copy text: ", err);
                    Swal.fire({ icon: "error", title: "Error", text: "Failed to copy message", theme: "dark" });
                }
            );
        });
...
dropZone.addEventListener('drop', (e) => {
    ...
    if (files.length > 1) {
        Swal.fire({ icon: 'error', title: 'Too many files', text: 'Only one file at a time!',theme: 'dark' });
        return;
    }
    handleFile(files[0]);
});
...
function handleFile(file) {
    ...
    if (!isValidType) {
        Swal.fire({ icon: 'error', title: 'Invalid file type', text: 'Please upload a PDF, TXT, or DOCX file.',theme: 'dark' });
        return;
    } else if (file.size > max_size_mb * 1024 * 1024) {
        Swal.fire({ icon: 'error', title: 'File too large', text: `Please upload a file smaller than ${max_size_mb} MB.`,theme: 'dark' });
        return;
    }
...
if (dropZoneNotes && fileInputNotes) {
    ...
    const validateAndProcess = (file) => {
        ...
        if (!validExt.includes(ext)) {
            Swal.fire({ icon: 'error', title: 'Invalid Format', text: 'Please use PDF, DOCX, or TXT.',theme: 'dark' });
            return;
        }
        if (file.size > 5 * 1024 * 1024) {
            Swal.fire({ icon: 'error', title: 'Too Large', text: 'Max file size is 5MB.',theme: 'dark' });
            return;
        }
